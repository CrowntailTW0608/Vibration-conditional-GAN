...
class cDCGAN():
    def __init__(self):
        ...
        self.model_path = rf"./exp/DCGAN/DCGAN_1DCNN_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def load_data(self):
        ...
        with open(r'./data\ary_500x_NORMAL_rpmGT50.pkl', 'rb')as f:
            dataset = pickle.load(f)

        x_train = dataset['data'][:, :, :500]  # [:,1:,1:4]
    ...
    def _build_gernerator(self, ):
        ...
        hid = Conv1DTranspose(128, kernel_size=5, strides=1, padding='same')(hid)
        hid = BatchNormalization(momentum=0.9)(hid)
        hid = LeakyReLU(alpha=0.1)(hid)
    ...
    def _build_discriminator(self):
        ...
        hid = Conv1D(32, kernel_size=3, strides=1, padding='same')(input_layer)
        hid = BatchNormalization(momentum=0.9)(hid)
        hid = LeakyReLU(alpha=0.1)(hid)
        ...
        hid = Conv1D(128, kernel_size=5, strides=1, padding='same')(hid)
        hid = BatchNormalization(momentum=0.9)(hid)
        hid = LeakyReLU(alpha=0.1)(hid)
        hid = Flatten()(hid)
        ...
        discriminator = Model(inputs=input_layer, outputs=out)

        discriminator.compile(optimizer=self.dis_opt, loss='binary_crossentropy')
    ...
    def _build_adversarial(self):
        ...
        adversarial = Model(inputs=gan_input, outputs=out)
        ...
        for epoch in range(epochs):
            ...
            with tqdm(total=len(slices), position=0, leave=True)as pbar:
                for i, slice_ in enumerate(slices):
                    ...
                    d_loss += self.discriminator.train_on_batch(legit_images, np.ones((len(X_train_batch)),) )

                    gen_noise = np.random.normal(-1, 1, (X_train_batch.shape[0], self.latence_dim[0]))  #
                    synthetic_images = self.generator.predict( gen_noise)
                    d_loss += self.discriminator.train_on_batch( synthetic_images,np.zeros((len(X_train_batch),)))

                    # train generator
                    self.discriminator.trainable = False
                    for j in range(2):
                        gen_noise = np.random.normal(0, 1, (X_train_batch.shape[0], self.latence_dim[0]))
                        g_loss += self.adversarial.train_on_batch(gen_noise, np.ones((len(X_train_batch)),))
            ...
            print('epoch: %d, [Discriminator :: d_loss: %f], [ Generator :: loss: %f]' % (epoch, d_loss, g_loss))
            ...
            if epoch % 10 == 0:
                ...
                noise = np.random.normal(-1, 1, (sample_per_class, 100))
                ...
                for c in range(sample_per_class):
                    ...
                    ax.set_title('real sampled data')
...
if __name__ == '__main__':
    ...
    cdcgan.train(epochs=2500, batch_size=600)
    # cdcgan.sample()
